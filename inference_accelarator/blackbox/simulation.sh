...
# Neural Network Simulation Runner with Neuron Count Filter
...
# Usage: ./script.sh [-n neuron_count1 neuron_count2 ...] [-h]
...
FIXED_TEST_LABELS="./shared_spike_mem/test_labels.txt"  # Fixed test labels path
...
usage() {
    echo "Usage: $0 [-n neuron_count1 neuron_count2 ...] [-h]"
    echo ""
    echo "Options:"
    echo "  -n  Specify neuron counts to process (e.g., -n 16 32 64)"
    echo "      If not specified, all configurations will be processed"
    echo "  -h  Show this help message"
    echo ""
    echo "Examples:"
    echo "  $0                    # Process all configurations"
    echo "  $0 -n 16 32 64       # Process only configurations with 16, 32, or 64 neurons"
    echo "  $0 -n 128            # Process only configurations with 128 neurons"
}
...
parse_arguments() {
    while [[ $# -gt 0 ]]; do
        case $1 in
            -n|--neurons)
                FILTER_ENABLED=true
                shift
                # Collect all neuron counts until next option or end
                while [[ $# -gt 0 && ! "$1" =~ ^- ]]; do
                    if [[ "$1" =~ ^[0-9]+$ ]]; then
                        NEURON_FILTER+=("$1")
                        shift
                    else
                        error "Invalid neuron count: $1 (must be a positive integer)"
                        exit 1
                    fi
                done
                ;;
    ...
    # Validate neuron filter
    if [[ "$FILTER_ENABLED" == true && ${#NEURON_FILTER[@]} -eq 0 ]]; then
...
# Function to check if a neuron count should be processed
should_process_neuron_count() {
    local neuron_count=$1
    
    # If no filter is enabled, process all
    if [[ "$FILTER_ENABLED" == false ]]; then
        return 0
    fi
    
    # Check if neuron count is in filter list
    for filter_count in "${NEURON_FILTER[@]}"; do
        if [[ "$neuron_count" == "$filter_count" ]]; then
            return 0
        fi
    done
...
# Function to check fixed test labels file
...
# Function to check inference data files
...
process_configuration() {
    ...
    # Check if this neuron count should be processed
    if ! should_process_neuron_count "$neurons"; then
        log "Skipping configuration: $config_name (neuron count $neurons not in filter)"
        return 0
    fi
    ...
    # Initialize accuracy results array
    local accuracy_t10="N/A"
    local accuracy_t20="N/A"
    local accuracy_t40="N/A"
    
    # Process different time step configurations
    local inference_time_steps=(10 20 40)
            ...
            if generate_spike_mem "$inference_dir/$test_values" "$inf_time"; then
                ...
                if run_simulation "$sim_name"; then
                    ...
                    if decode_results "$sim_name"; then
                        ...
                        # Calculate accuracy using fixed test labels
                        local accuracy
                        if calculate_accuracy "$sim_name"; then
                            accuracy="$ACCURACY_RESULT"
                            # Store accuracy based on inference time steps
                            case $inf_time in
    ...
    # Write single row to CSV with all accuracy results - CLEAN FORMAT
    write_to_csv "$config_name,$layers,$neurons,$time_steps,$accuracy_t10,$accuracy_t20,$accuracy_t40"
...
initialize_shared_spike_mem() {
    ...
    # Check if fixed test labels file exists
    if ! check_fixed_test_labels; then
        error "Fixed test labels file is required but not found: $FIXED_TEST_LABELS"
        return 1
    fi
    ...
    while IFS= read -r -d '' dir; do
        if [[ -d "$dir/inference_data" ]]; then
            # Check if this configuration should be processed (neuron filter)
...
# Function to filter configuration directories based on neuron count
filter_config_dirs() {
    local all_dirs=("$@")
    local filtered_dirs=()
...
main() {
    ...
    if [[ "$FILTER_ENABLED" == true ]]; then
        log "Neuron count filter enabled: ${NEURON_FILTER[*]}"
    else
        log "Processing all configurations (no neuron filter)"
    fi
    ...
    # Initialize results file with header - CLEAN FORMAT
    echo "Configuration,Layers,Neurons,TimeSteps,Accuracy_T10,Accuracy_T20,Accuracy_T40" > "$RESULTS_FILE"
    ...
    local all_config_dirs=()
    while IFS= read -r -d '' dir; do
        all_config_dirs+=("$dir")
    done < <(find "$BASE_DIR" -maxdepth 1 -type d -name "l*_n*_t*" -print0 | sort -z)
    
    if [[ ${#all_config_dirs[@]} -eq 0 ]]; then
        error "No configuration directories found in $BASE_DIR"
        exit 1
    fi
    
    # Filter configuration directories based on neuron count
    local config_dirs=()
    if [[ "$FILTER_ENABLED" == true ]]; then
        readarray -t config_dirs < <(filter_config_dirs "${all_config_dirs[@]}")
    else
        config_dirs=("${all_config_dirs[@]}")
    fi
    
    if [[ ${#config_dirs[@]} -eq 0 ]]; then
        error "No configuration directories match the specified neuron count filter: ${NEURON_FILTER[*]}"
        exit 1
    fi
    
    log "Found ${#all_config_dirs[@]} total configuration directories"
    log "Processing ${#config_dirs[@]} configuration directories after filtering"
