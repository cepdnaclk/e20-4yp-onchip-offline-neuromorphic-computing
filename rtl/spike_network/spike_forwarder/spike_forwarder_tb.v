`timescale 1ns/1ps
`include "spike_forwarder.v"

module spike_forwarder_tb;
    ...
    reg router_mode;
    reg load_row;
    reg [num_ports:0] row_index;
    reg [num_ports:0] forwarding_row;
    ...
    initial begin
        ...
        #10 load_row = 1;
        row_index = 1;
        forwarding_row = 5'b10100; 
        #10;
        load_row = 0;

        #10 load_row = 1;
        row_index = 2;
        forwarding_row = 5'b10011; 
        #10;
        load_row = 0;

        #10 load_row = 1;
        row_index = 3;
        forwarding_row = 5'b00001; 
        #10;
        load_row = 0;
        ...
        for (debug_i = 0; debug_i < num_ports + 1; debug_i = debug_i + 1) begin
            $display("  [%0d] = %b", debug_i, dut.forwarding_map[debug_i]);
        end
    ...
    task print_inflight;
        ...
        begin
            ...
            for (j = 0; j < num_ports + 1; j = j + 1)
                $write("[%0d]=%0d ", j, dut.inflight_count[j]);
            $write("\n");
            $write("time=%0t fifo_count = ", $time);
            for (j = 0; j < num_ports; j = j + 1)
                $write("[%0d]=%0d ", j, dut.fifo_count_out[j]);
            $write("\n");
        end
    endtask
    ...
    initial begin
        $dumpfile("spike_forwarder_tb.vcd");
        $dumpvars(0, spike_forwarder_tb);
    end
